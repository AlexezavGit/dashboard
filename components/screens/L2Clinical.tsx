...
const DROP_STAGES = [
  ...
  { stage: { uk: 'Ціль', en: 'Target' }, pct: 80, note: { uk: 'mhGAP повний курс', en: 'mhGAP full course' }, isTarget: true },
];
...
const DRIVERS = [
  ...
  { icon: '⊜', val: { uk: '260K НСЗУ пацієнтів', en: '260K NHSU patients' }, desc: { uk: 'лише пацієнти на первинній ланці видимі НСЗУ', en: 'only primary care patients visible to NHSU' } },
];
...
export const L2Clinical: React.FC<Props> = ({ lang, nav }) => (
  ...
  >
    ...
    <NavBar
      lang={lang}
      onBack={nav.back}
      title={{ uk: 'Clinical · Завершуваність реабілітації', en: 'Clinical · Rehabilitation completion' }}
    ...
    <div className="flex-1 grid grid-cols-2 gap-5 px-6 pb-4 pt-3 min-h-0">
      ...
      <div className="flex flex-col gap-4">
        <div className="rounded-2xl p-5 flex-shrink-0"
          ...
          <div className="text-[10px] font-mono uppercase tracking-wider mb-1" style={{ color: '#ff7b6e' }}>
            MHEI · Clinical · w25%
          </div>
          <div className="ds-display font-black leading-none" style={{ fontSize: '62px', color: '#ff7b6e', textShadow: '0 0 40px rgba(255,123,110,0.5)' }}>
            ~40%
          </div>
          ...
          <div className="mt-3 px-3 py-2 rounded-lg text-[11px] font-semibold ds-display inline-block"
            style={{ background: 'rgba(255,123,110,0.12)', color: '#ff7b6e', border: '1px solid rgba(255,123,110,0.25)' }}>
            {t('Ціль: ≥80% · MHEI-внесок: 0/25 балів зараз', 'Target: ≥80% · MHEI contribution: 0/25 pts now', lang)}
          </div>
        </div>
            ...
            {DROP_STAGES.map((s) => (
              <div key={s.stage.uk}>
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[11px] ds-display font-semibold" style={{ color: s.isTarget ? 'var(--color-ds-gold)' : 'var(--color-ds-text)' }}>
                    {s.stage[lang]}
                  </span>
                  <span className="text-[12px] font-bold ds-display" style={{ color: s.isTarget ? 'var(--color-ds-gold)' : '#ff7b6e' }}>
                    {s.pct}%
                  </span>
                </div>
                ...
                <div className="text-[9px] ds-body mt-0.5" style={{ color: 'var(--color-ds-muted)' }}>{s.note[lang]}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {DRIVERS.map((d, i) => (
              <div key={i} className="flex gap-3 items-start">
                ...
                <div>
                  <div className="text-[12px] font-bold ds-display" style={{ color: 'var(--color-ds-text)' }}>{d.val[lang]}</div>
        ...
        <div className="flex-1 rounded-2xl p-5"
          ...
          <div className="mt-4 pt-3" style={{ borderTop: '1px solid var(--color-ds-border)' }}>
            <div className="text-[9px] font-mono" style={{ color: 'var(--color-ds-muted)' }}>
              {t('Джерело: NHSU open data 2025 · Lancet 2024 · mhGAP Implementation Guide v3', 'Source: NHSU open data 2025 · Lancet 2024 · mhGAP Implementation Guide v3', lang)}
            </div>
          </div>
        </div>
      </div>
