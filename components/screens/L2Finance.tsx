...
// 4-node path: Allocation → Provider → Session → Result
const CHAIN_NODES = [
  {
    id: 'alloc',
...
// Breaks between 4 nodes: [0,1], [1,2], [2,3]
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 12, color: '#ff7b6e', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              {b.label[lang]}
            </span>
          </div>
          <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 16, color: 'var(--color-ds-text)', lineHeight: 1.3 }}>
            {b.title[lang]}
          </div>
          <div style={{ flex: 1 }} />
        ...
        <div style={{
          ...
          <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 13, color: '#ff7b6e', flexShrink: 0 }}>
            {b.title[lang]}
          </div>
...
const DonorChart: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  return (
    <div style={{ flex: 1, borderRadius: 12, padding: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(200,164,92,0.2)' }}>
      ...
      <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 9, color: 'var(--color-ds-muted)', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 8 }}>
...
const BudgetChart: React.FC<{ lang: Language }> = ({ lang }) => {
  ...
  return (
    <div style={{ flex: 1, borderRadius: 12, padding: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,123,110,0.2)' }}>
      <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 12, color: 'var(--color-ds-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
        {lang === 'uk' ? 'Бюджет МЗ — структурна інверсія (2025)' : 'MH budget — structural inversion (2025)'}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 10 }}>
        {data.map((d) => (
          <div key={d.name}>
      ...
      <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 9, color: '#ff7b6e', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 8, marginBottom: 8 }}>
        {lang === 'uk' ? '↑ 5× стаціонар vs реальна потреба 11%' : '↑ 5× inpatient vs actual need 11%'}
      </div>
      <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 9, color: 'var(--color-ds-muted)' }}>
...
const OutcomesChart: React.FC<{ lang: Language }> = ({ lang }) => {
  return (
    <div style={{ flex: 1, borderRadius: 12, padding: 16, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(200,164,92,0.2)' }}>
      <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 12, color: 'var(--color-ds-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>
        {lang === 'uk' ? 'Міжнародна допомога в обмін на результат' : 'Intl. aid in exchange for results'}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 10 }}>
        {WB_PROGRAMS.map((p) => (
          <div key={p.name}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 10, color: 'var(--color-ds-muted)', minWidth: 60 }}>{p.total}</span>
            </div>
          </div>
        ))}
      </div>
...
export const L2Finance: React.FC<Props> = ({ lang, nav }) => (
  ...
  >
    ...
    <div className="flex-1 flex flex-col gap-4 px-5 pb-4 min-h-0">

      {/* Resource path chain — 4 nodes */}
      <motion.div
        initial={{ opacity: 0, y: -6 }}
        animate={{ opacity: 1, y: 0 }}
        className="flex-shrink-0"
      >
        <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 13, color: 'var(--color-ds-gold)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 3 }}>
            ...
            return (
              <React.Fragment key={node.id}>
                ...
                >
                  ...
                  <div className="ds-body mt-1" style={{ fontSize: '10px', color: 'var(--color-ds-muted)', lineHeight: 1.4 }}>
                    {node.sub[lang]}
                  </div>
                </motion.div>
      ...
      <div className="flex flex-col gap-2 min-h-0 flex-1" style={{ minHeight: 140 }}>
        <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 13, color: '#ff7b6e', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          {lang === 'uk' ? 'ТОЧКИ РОЗЄДНАННЯ' : 'DISCONNECT POINTS'}
        </div>
        <div
      ...
      >
        <DonorChart lang={lang} />
        <BudgetChart lang={lang} />
        <OutcomesChart lang={lang} />
      </motion.div>
    </div>
  </div>
);
