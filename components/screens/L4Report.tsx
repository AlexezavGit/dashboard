...
export const L4Report: React.FC<Props> = ({ lang, onBack }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] overflow-hidden flex flex-col" style={{ background: 'var(--color-ds-bg)', color: 'var(--color-ds-text)' }}>
      ...
      {/* Top bar */}
      <div className="flex-shrink-0 flex items-center gap-3 px-5 pt-3 pb-3" style={{ borderBottom: '1px solid var(--color-ds-border)' }}>
        <button
          onClick={onBack}
          className="flex items-center gap-2 px-3 py-2 rounded-xl font-bold ds-display transition-all flex-shrink-0"
          style={{ background: 'rgba(200,164,92,0.18)', border: '2px solid var(--color-ds-gold)', color: 'var(--color-ds-gold)', fontSize: 12 }}
        ...
        <div className="text-[13px] font-bold ds-display" style={{ color: 'var(--color-ds-gold)' }}>
          {t('Аналітичний звіт FEEL Again', 'FEEL Again Analytical Report')}
        </div>
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        {/* Sidebar */}
        <div className="flex-shrink-0 w-[190px] flex flex-col py-3 px-2 gap-1 overflow-y-auto hidden sm:flex"
          style={{ borderRight: '1px solid var(--color-ds-border)', background: 'rgba(0,0,0,0.15)', scrollbarWidth: 'thin' }}>
          {CHAPTERS.map((c, i) => {
            ...
            return (
              <button
                ...
                style={{
                  background: active ? 'rgba(200,164,92,0.15)' : 'transparent',
                  border: active ? '1px solid rgba(200,164,92,0.35)' : '1px solid transparent',
                }}
              >
                <span className="text-[9px] font-mono mt-0.5 flex-shrink-0"
                  style={{ color: active ? 'var(--color-ds-gold)' : 'var(--color-ds-muted)' }}>
          ...
          <div className="mt-auto pt-4 px-1">
            <div className="text-[8px] font-mono mb-1.5" style={{ color: 'var(--color-ds-muted)' }}>
              {t('Прогрес', 'Progress')}
            </div>
            <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <div className="h-full rounded-full transition-all duration-500"
                style={{ width: `${((chapterIdx + 1) / CHAPTERS.length) * 100}%`, background: 'var(--color-ds-gold)' }} />
            </div>
          </div>
        </div>

        {/* Mobile chapter tabs (visible only on small screens) */}
        <div className="flex-shrink-0 sm:hidden overflow-x-auto flex items-center gap-1 px-3 py-2 absolute top-[52px] left-0 right-0 z-10"
          style={{ background: 'var(--color-ds-bg)', borderBottom: '1px solid var(--color-ds-border)' }}>
          {CHAPTERS.map((c) => (
            <button key={c.id} onClick={() => setChapter(c.id)}
              className="flex-shrink-0 px-2 py-1 rounded text-[9px] font-bold font-mono uppercase tracking-wide"
              style={chapter === c.id
                ? { background: 'var(--color-ds-gold)', color: '#0a1628' }
                : { color: 'var(--color-ds-muted)', border: '1px solid var(--color-ds-border)' }}>
              {String(CHAPTERS.findIndex(x => x.id === c.id) + 1)}
            </button>
          ))}
        </div>

        {/* Content */}
        <div className="flex-1 overflow-y-auto px-6 py-8 max-w-[760px] space-y-10">
            ...
            <Section title={t('Потік даних: гуманітарна → державна зона', 'Data flow: humanitarian → state zone')} color="var(--color-ds-teal)">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center text-[12px]">
                <div className="rounded-xl p-5 space-y-3" style={{ border: '1px solid var(--color-ds-teal)33', background: 'var(--color-ds-teal)08' }}>
                  ...
                  {arch.humanitarian.sources.map((s: string) => (<div key={s} className="text-[11px]" style={{ color: 'var(--color-ds-muted)' }}>· {s}</div>))}
                  {arch.humanitarian.highlights.map((h: string) => (<div key={h} className="text-[11px] font-bold" style={{ color: 'var(--color-ds-teal)' }}>{h}</div>))}
                </div>
                <div className="rounded-xl p-5 space-y-2" style={{ border: '1px solid var(--color-ds-gold)33', background: 'var(--color-ds-gold)08' }}>
                  ...
                  {arch.middleware.components.map((c: string) => (<div key={c} className="text-[11px]" style={{ color: 'var(--color-ds-muted)' }}>· {c}</div>))}
                </div>
                <div className="rounded-xl p-5 space-y-2" style={{ border: '1px solid #3B82F633', background: '#3B82F608' }}>
                  ...
                  {arch.state.systems.map((s: any) => (<div key={s.name} className="text-[11px] font-bold" style={{ color: s.color }}>· {s.name} {s.sub && <span className="font-normal opacity-60">({s.sub})</span>}</div>))}
                </div>
              </div>
            </Section>
          )}
                    ...
                    <div>
                      ...
                      {(s.url ?? s.link) && (
                        <a href={s.url ?? s.link} target="_blank" rel="noopener noreferrer"
                          className="ml-2 font-mono underline" style={{ color: 'var(--color-ds-teal)' }}>
                          {s.url ?? s.link}
                        </a>
                      )}
