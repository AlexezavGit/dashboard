...
const PIPELINE = [
  { label: { uk: 'Сертифіковано mhGAP', en: 'mhGAP certified' }, val: '~25K', color: '#a78bfa' },
  { label: { uk: 'Активних 12 міс.', en: 'Active at 12 mo.' }, val: '~10K', color: '#a78bfa', dim: true },
  { label: { uk: 'Видимих у реєстрі', en: 'Visible in registry' }, val: '~6K', color: '#ff9966', dim: true },
  { label: { uk: 'Верифікованих НСЗУ', en: 'NHSU-verified' }, val: '~4K', color: '#ff7b6e', dim: true },
];
...
const BARRIERS = [
  ...
  { uk: 'Imitation Index: практик бачить, що інші не практикують → зупиняється', en: 'Imitation Index: practitioner sees peers not practicing → stops' },
];
...
export const L2Sustain: React.FC<Props> = ({ lang, nav }) => (
  ...
  >
    ...
    <NavBar
      lang={lang}
      onBack={nav.back}
      title={{ uk: 'Sustainable Dev · Конверсія навчання', en: 'Sustainable Dev · Training conversion' }}
    ...
    <div className="flex-1 grid grid-cols-2 gap-5 px-6 pb-4 pt-3 min-h-0">
      ...
      <div className="flex flex-col gap-4">
        <div className="rounded-2xl p-5 flex-shrink-0"
          ...
          <div className="text-[10px] font-mono uppercase tracking-wider mb-1" style={{ color: '#a78bfa' }}>
            MHEI · Sustainable Dev · w15%
          </div>
          <div className="ds-display font-black leading-none" style={{ fontSize: '62px', color: '#a78bfa', textShadow: '0 0 40px rgba(167,139,250,0.5)' }}>
            ~35%
          </div>
          ...
          <div className="mt-3 px-3 py-2 rounded-lg text-[11px] font-semibold ds-display inline-block"
            ...
            {t('Ціль: ≥70% · зараз: 7.5/15 балів MHEI', 'Target: ≥70% · now: 7.5/15 MHEI pts', lang)}
          </div>
        </div>
            ...
            {PIPELINE.map((p, i) => (
              <div key={i}>
                <div className="flex justify-between mb-1">
                  <span className="text-[11px] ds-display font-semibold" style={{ color: p.dim ? 'var(--color-ds-muted)' : 'var(--color-ds-text)' }}>
                    {p.label[lang]}
                  </span>
                  <span className="text-[13px] font-bold ds-display" style={{ color: p.color, opacity: p.dim ? 0.7 : 1 }}>
                    {p.val}
                  </span>
                </div>
                <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
                  <div className="h-1.5 rounded-full" style={{
                    width: i === 0 ? '100%' : i === 1 ? '40%' : i === 2 ? '24%' : '16%',
                    background: p.color,
                    opacity: p.dim ? 0.5 : 0.9,
                  }} />
                </div>
              </div>
            ))}
      ...
      {/* Right — barriers + FEEL Again fix */}
      <div className="flex flex-col gap-4">
        <div className="rounded-2xl p-5"
          style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--color-ds-border)' }}>
          <div className="text-[10px] font-mono uppercase tracking-wider mb-3" style={{ color: 'var(--color-ds-muted)' }}>
            {t('Бар\'єри конверсії', 'Conversion barriers', lang)}
          </div>
          <div className="space-y-3">
            {BARRIERS.map((b, i) => (
              <div key={i} className="flex gap-2 items-start">
                <div className="text-[14px] leading-none mt-0.5" style={{ color: '#ff7b6e' }}>⊘</div>
                <div className="text-[11px] ds-body" style={{ color: 'var(--color-ds-text)' }}>{b[lang]}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 rounded-2xl p-5"
          ...
          <div className="mt-4 pt-3" style={{ borderTop: '1px solid var(--color-ds-border)' }}>
            <div className="text-[9px] font-mono" style={{ color: 'var(--color-ds-muted)' }}>
              {t('Джерело: NHSU open data 2025 · WHO mhGAP 2023 · Train for Care оцінка', 'Source: NHSU open data 2025 · WHO mhGAP 2023 · Train for Care assessment', lang)}
            </div>
          </div>
        </div>
      </div>
