...
const ProgramConstellation: React.FC<{ lang: Language }> = ({ lang }) => {
  const items = [
    {
      id: 'dashboard',
      title: lang === 'uk' ? 'Дашборд як control layer' : 'Dashboard as control layer',
      subtitle: lang === 'uk' ? 'Операційна аналітика' : 'Operational intelligence',
      description: lang === 'uk'
        ? 'Це не просто звіт. Це центр управління для потреб, охоплення, фінансових потоків, ризиків і розривів між гуманітарним, державним і фінансовим контекстом.'
        : 'This is not just a report. It is the control layer for need, coverage, funding flow, risk, and the gaps between humanitarian, public, and financial systems.',
      accent: 'var(--color-ds-teal)',
      icon: LayoutDashboard,
    },
    {
      id: 'site',
      title: lang === 'uk' ? 'Сайт як доказ і легітимація' : 'Website as proof and legitimacy',
      subtitle: lang === 'uk' ? 'Комунікація й легітимація' : 'Narrative & legitimacy',
      description: lang === 'uk'
        ? 'Публічний простір, що пояснює модель, результати й шляхи участі для держави, донорів, партнерів і громадян.'
        : 'The public layer that explains the model, outcomes, and pathways to participation for government, donors, partners, and citizens.',
      accent: 'var(--color-ds-gold)',
      icon: Globe,
    },
    {
      id: 'cabinets',
      title: lang === 'uk' ? 'Кабінети як впровадження' : 'Cabinets as execution',
      subtitle: lang === 'uk' ? 'Операційний рівень' : 'Operational layer',
      description: lang === 'uk'
        ? 'Приватний простір для команд і користувачів, де дані переходять у заявки, верифікацію, фінансові сигнали й щоденні дії.'
        : 'The private space where data turns into requests, verification, financial signals, and day-to-day action for teams and users.',
      accent: 'var(--color-ds-orange)',
      icon: Users,
    },
  ];
      ...
      {items.map((item, index) => {
        ...
        return (
          <motion.div
            ...
            className="ds-puzzle-box p-5 relative overflow-hidden"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <div className="p-2 rounded-lg border" style={{ borderColor: item.accent, color: item.accent }}>
                  <Icon className="w-4 h-4" />
                </div>
                <div>
                  <div className="cyber-label text-[10px] uppercase tracking-[0.2em]" style={{ color: 'var(--color-ds-muted)' }}>{item.subtitle}</div>
                  <div className="text-[14px] font-bold" style={{ color: 'var(--color-ds-text)' }}>{item.title}</div>
                </div>
              </div>
              <div className="text-[11px] font-mono" style={{ color: item.accent }}>0{index + 1}</div>
            </div>
            <p className="text-[12px] leading-relaxed" style={{ color: 'var(--color-ds-text)' }}>{item.description}</p>
          </motion.div>
        );
      })}
    </div>
...
const OperationalLoop: React.FC<{ lang: Language }> = ({ lang }) => {
  const steps = [
    {
      title: lang === 'uk' ? '1. Виявити розрив' : '1. Detect the gap',
      body: lang === 'uk'
        ? 'Показуємо, де потреба перевищує охоплення, де ресурси розпорошені, а де ефективність не видно.'
        : 'We show where demand exceeds coverage, where resources are fragmented, and where performance remains invisible.',
      accent: 'var(--color-ds-teal)',
    },
    {
      title: lang === 'uk' ? '2. Активувати дію' : '2. Activate action',
      body: lang === 'uk'
        ? 'Транслюємо сигнал у стратегію, заявки, працівників, послуги та фінансові пріоритети.'
        : 'We translate the signal into strategy, requests, staffing, services, and financing priorities.',
      accent: 'var(--color-ds-gold)',
    },
    {
      title: lang === 'uk' ? '3. Верифікувати результат' : '3. Verify outcome',
      body: lang === 'uk'
        ? 'Пов’язуємо дані з реальним виконанням, щоб управління могло бачити не лише витрати, а й ефект.'
        : 'We connect raw data to real delivery so management can see not only cost, but also effect.',
      accent: 'var(--color-ds-orange)',
    },
  ];

  return (
    <div className="mb-8 rounded-2xl border p-5" style={{ borderColor: 'color-mix(in srgb, var(--color-ds-teal) 22%, transparent)', background: 'color-mix(in srgb, var(--color-ds-teal) 9%, transparent)' }}>
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between mb-4">
        <div>
          <div className="cyber-label text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: 'var(--color-ds-teal)' }}>
            {lang === 'uk' ? 'ОПЕРАТИВНИЙ ЦИКЛ' : 'OPERATING LOOP'}
          </div>
          <div className="text-[16px] font-bold" style={{ color: 'var(--color-ds-text)' }}>
            {lang === 'uk'
              ? 'Дашборд повинен вести не лише до розуміння, а й до дії.'
              : 'The dashboard should lead not only to understanding, but to action.'}
          </div>
        </div>
        <div className="text-[11px] leading-relaxed max-w-xl" style={{ color: 'var(--color-ds-muted)' }}>
          {lang === 'uk'
            ? 'Це макроопераційна логіка: сприйняти сигнал, розгорнути сервіс, перевірити ефект і закріпити фінансовий контроль.'
            : 'This is the macro-operational logic: perceive the signal, deploy the service, verify the effect, and reinforce financial control.'}
        </div>
      </div>
      <div className="grid gap-3 md:grid-cols-3">
        {steps.map((step, index) => (
          <div key={step.title} className="rounded-xl border p-4" style={{ borderColor: 'color-mix(in srgb, ' + step.accent + ' 24%, transparent)', background: 'color-mix(in srgb, ' + step.accent + ' 8%, transparent)' }}>
            <div className="text-[11px] font-mono font-bold mb-2" style={{ color: step.accent }}>{step.title}</div>
            <p className="text-[11px] leading-relaxed" style={{ color: 'var(--color-ds-text)' }}>{step.body}</p>
          </div>
        ))}
      </div>
    </div>
          ...
          <div className="flex-1 min-h-screen">
            <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pb-12" id="hero-top">
              ...
              <div className="flex-shrink-0 pt-4 pb-3 mb-6" style={{ borderBottom: '1px solid var(--color-ds-border)' }}>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold ds-display transition-all flex-shrink-0 w-full sm:w-auto"
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:ml-auto">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 px-4 py-1.5 rounded-lg text-[11px] font-bold ds-display flex-shrink-0 transition-all bg-orange-500 text-slate-900 hover:bg-orange-400 shadow-[0_0_15px_rgba(245,158,11,0.3)] animate-pulse w-full sm:w-auto"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold ds-display flex-shrink-0 transition-all w-full sm:w-auto"
                ...
                <div className="mt-4 ml-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    <h2 className="text-[22px] sm:text-[28px] font-bold ds-display leading-tight tracking-tight" style={{ color: 'var(--color-ds-teal)' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6 gap-1.5 mb-8">
              {[
                {
                  id: 'gap-covered',
                  val: '0.28%',
            ...
            <ProgramConstellation lang={lang} />
            <OperationalLoop lang={lang} />

            <div className="mb-8 grid gap-4 lg:grid-cols-3">
              {[
                {
                  title: lang === 'uk' ? 'Для державного управління' : 'For public management',
                  text: lang === 'uk'
                    ? 'Єдиний макроопераційний контур для оцінки потреб, витрат, ризиків і розривів між програмними інструментами.'
                    : 'A single macro-operational view for assessing need, cost, risk, and the gaps between programme instruments.',
                  accent: 'var(--color-ds-teal)',
                  icon: ShieldCheck,
                },
                {
                  title: lang === 'uk' ? 'Для донорів і партнерів' : 'For donors and partners',
                  text: lang === 'uk'
                    ? 'Чітка візуалізація ефективності, фінансового сліду, результатів і обґрунтування для подальших інвестицій.'
                    : 'A clear view of effectiveness, financial traceability, outcomes, and the case for continued investment.',
                  accent: 'var(--color-ds-gold)',
                  icon: TrendingUp,
                },
                {
                  title: lang === 'uk' ? 'Для операційних команд' : 'For operational teams',
                  text: lang === 'uk'
                    ? 'Переходить від аналітики до дій: заявки, верифікація, синхронізація даних і повсякденне виконання.'
                    : 'Moves from analysis to action: requests, verification, data sync, and daily execution.',
                  accent: 'var(--color-ds-orange)',
                  icon: BookOpen,
                },
              ].map((item, index) => {
                ...
                return (
                  <motion.div
                    ...
                    style={{ borderColor: 'color-mix(in srgb, ' + item.accent + ' 24%, transparent)', background: 'color-mix(in srgb, ' + item.accent + ' 8%, transparent)' }}
                  >
                    <div className="flex items-center gap-2 mb-3">
                      <div className="p-2 rounded-lg border" style={{ borderColor: item.accent, color: item.accent }}>
                        <Icon className="w-4 h-4" />
                      </div>
                      <div className="text-[13px] font-bold" style={{ color: 'var(--color-ds-text)' }}>{item.title}</div>
                    </div>
                    <p className="text-[12px] leading-relaxed" style={{ color: 'var(--color-ds-text)' }}>{item.text}</p>
                  </motion.div>
                );
              })}
            </div>

            <div className="mb-8 rounded-2xl border p-5" style={{ borderColor: 'color-mix(in srgb, var(--color-ds-teal) 28%, transparent)', background: 'color-mix(in srgb, var(--color-ds-teal) 10%, transparent)' }}>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <div className="cyber-label text-[10px] uppercase tracking-[0.25em] mb-2" style={{ color: 'var(--color-ds-teal)' }}>
                    {lang === 'uk' ? 'ЦЕ НЕ ЛИШЕ ДАШБОРД' : 'THIS IS MORE THAN A DASHBOARD'}
                  </div>
                  <div className="text-[18px] font-bold" style={{ color: 'var(--color-ds-text)' }}>
                    {lang === 'uk' ? 'MHPSS + fintech + цифровий сервісний контур' : 'MHPSS + fintech + the digital service loop'}
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <a href="https://feelagain.me" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-[11px] font-bold ds-display transition-all" style={{ background: 'rgba(200,164,92,0.16)', color: 'var(--color-ds-gold)', border: '1px solid rgba(200,164,92,0.35)' }}>
                    <ExternalLink className="w-3.5 h-3.5" />
                    {lang === 'uk' ? 'Відкрити сайт FEEL Again' : 'Open FEEL Again website'}
                  </a>
                  <a href="https://terminal.feelagain.me" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-3 py-2 rounded-lg text-[11px] font-bold ds-display transition-all" style={{ background: 'rgba(0,210,255,0.14)', color: 'var(--color-ds-teal)', border: '1px solid rgba(0,210,255,0.3)' }}>
                    <Users className="w-3.5 h-3.5" />
                    {lang === 'uk' ? 'Відкрити робочі кабінети' : 'Open user cabinets'}
                  </a>
                </div>
              </div>
              <p className="mt-3 text-[12px] leading-relaxed" style={{ color: 'var(--color-ds-text)' }}>
                {lang === 'uk'
                  ? 'Дашборд показує стан системи, сайт дає їй пояснення, а кабінети переводять її в дії. Саме ця тріада робить програму не лише аналітичною, а й операційно живою.'
                  : 'The dashboard shows the state of the system, the website explains it, and the cabinets translate it into action. This triad makes the programme not only analytical, but operationally alive.'}
              </p>
            </div>
            ...
            <div className="mb-8 px-5 py-4 rounded-xl flex items-start gap-4" style={{ border: '1px solid var(--color-ds-gold-dim)', background: 'var(--color-ds-gold-dim)' }}>
              ...
              <p className="text-[13px] leading-relaxed font-mono" style={{ color: 'var(--color-ds-text)' }}>
                {lang === 'uk'
                  ? 'FEEL Again — це цифрова шина між MHPSS-даними, фінтех-логікою та операційним середовищем сервісів. Дашборд є ядром для бачення розриву й ефективності, сайт пояснює програму для партнерів і громадян, а робочі кабінети переводять дані в реальні дії, заяви й верифікацію. ❤️‍🔥 Gap: послуги існують, але поки не повністю візуалізовані для управління.'
                  : 'FEEL Again is the digital bridge between MHPSS data, fintech logic, and the operating environment of services. The dashboard is the core for seeing the gap and performance, the website explains the programme to partners and citizens, and the user cabinets turn data into real actions, requests, and verification. ❤️‍🔥 Gap: services exist, but are not yet fully visible to management.'}
              </p>
            </div>
                              ...
                              <div className="space-y-4">
                                {ADMIN_BURDEN(lang).map((item, idx) => (
                                  <div key={idx} className="flex gap-3 items-start border-b border-cyber-border/30 pb-3">
                                    <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest shrink-0 mt-1 font-mono ${item.color === 'red' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' :
